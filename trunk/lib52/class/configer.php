...
// +--------------------------------------------------------------------+
//
// $Id: configer.php 59 2010-05-05 02:02:04Z zhangxc83 $

class Aleafs_Lib_Configer
{
    ...
    public static function &instance($key)
    {
        ...
        if (!isset(self::$objs[$key])) {
            if (!isset(self::$rules[$key])) {
                throw new Aleafs_Lib_Exception(sprintf('Undefined config object named as "%s"', $key));
            }
    ...
    private static function loadUrl($url)
    {
        ...
        $url = parse_url($url);
        $tmp = explode('.', trim($url['path']));
        $cls = empty($url['scheme']) ? array_pop($tmp) : $url['scheme'];
        $cls = sprintf('%s_%s', __CLASS__, ucfirst(strtolower($cls)));
        $obj = new $cls($url);
